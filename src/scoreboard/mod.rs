...
pub struct ScoreboardController {
    client: Arc<Mutex<ScoreboardClient>>,
...
impl ScoreboardController {
    ...
    pub fn new(address: String, card_id: u8) -> Self {
        let client = ScoreboardClient::new(address, card_id);
        let layout = ScoreboardLayout::standard_224x32();
        
        Self {
            client: Arc::new(Mutex::new(client)),
            layout,
            state: Arc::new(Mutex::new(ScoreboardState::default())),
        }
    }
    ...
    pub async fn update_display(&self) -> Result<()> {
        ...
        // Update team names
        self.send_text_command(&mut client, windows::HOME_NAME, &state.home_team, Color::WHITE).await?;
        self.send_text_command(&mut client, windows::AWAY_NAME, &state.away_team, Color::WHITE).await?;
        
        // Update scores
        self.send_text_command(&mut client, windows::HOME_SCORE, &state.home_score.to_string(), Color::GREEN).await?;
        self.send_text_command(&mut client, windows::AWAY_SCORE, &state.away_score.to_string(), Color::GREEN).await?;
        ...
        self.send_text_command(&mut client, windows::TIMER, &timer_text, timer_color).await?;
